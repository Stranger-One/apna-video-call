...
let [videos, setVideos] = useState([]); //other users videos
        ...
        for (let id in connections) {
            ...
            connections[id].createOffer().then((description) => {
                ...
                connections[id].setLocalDescription(description)
                    .then(() => {
                        socketRef.current.emit('signal', id, JSON.stringify({ 'sdp': connections[id].localDescription }))//socketIdRef
                    })
                    .catch(e => console.log(e))
            })
        }

        stream.getTracks().forEach(track => track.onended = () => { //track band padd gaya toh blck silence create karna padega
            setVideo(false);
            setAudio(false);
    ...
    let gotMessageFromServer = (fromId, message) => {
        ...
        //neha se reply
        if (fromId !== socketIdRef.current) {//fromId hein wo mein nhi hun mein apne aap ko kaise msg karu
            if (signal.sdp) { //sdp means session description protocol fromId meand id2
                connections[fromId].setRemoteDescription(new RTCSessionDescription(signal.sdp)).then(() => {//sessionDescription yaha padh rhe hein
            ...
            if (signal.ice) {//initial connection establishment ke liye
        ...
        socketRef.current.on('connect', () => {
            ...
            socketRef.current.on('user-joined', (id, clients) => {
                clients.forEach((socketListId) => {
                    ...
                    connections[socketListId].onicecandidate = function (event) {//ice meant protocol ye asal mein ek client and dusre client ke beech mein direct connection banane mein madad karta hai
                    ...
                    connections[socketListId].onaddstream = (event) => {
                        ...
                        let videoExists = videoRef.current.find(video => video.socketId === socketListId); //agar video exist karta hein toh uska stream update karenge nhi toh naya video create karenge
                        ...
                        if (videoExists) {
                            ...
                            setVideos(videos => {
                                const updatedVideos = videos.map(video =>
                                    video.socketId === socketListId ? { ...video, stream: event.stream } : video  //ye async function hein toh ismein setVideos use karna padega aur hum direct nhi kar sakte videoRef ka use karni padegi
                                );
                    ...
                    } else {//jab bhikoi video off karega black screen dikhane ke liye
                        let blackSilence = (...args) => new MediaStream([black(...args), silence()])
                        window.localStream = blackSilence()
                        connections[socketListId].addStream(window.localStream)
                    }
                ...
                if (id === socketIdRef.current) {
                    for (let id2 in connections) { //connections mein sabka socket id list ho rha hein
                                ...
                                {messages.length !== 0 ? messages.map((item, index) => {
                                    ...
                                    return (
                                        <div style={{ marginBottom: "20px" }} key={index}>
                                            <p style={{ fontWeight: "bold",color:"brown" }}>{item.sender}</p>
                                            <p>{item.data}</p>
                                        </div>
                                    )
                                }) : <p>No Messages Yet</p>}
